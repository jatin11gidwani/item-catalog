...
def newCatalogItem(category_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add catalog items to this category. Please create your own category in order to add items.');}</script><body onload='myFunction()''>"  # noqa
    if request.method == 'POST':
            newItem = CatalogItem(
...
# Edit a catalog item
@app.route(
          '/category/<int:category_id>/catalog/<int:catalog_id>/edit',
          methods=['GET', 'POST'])
def editCatalogItem(category_id, catalog_id):
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit catalog items to this category. Please create your own category in order to edit items.');}</script><body onload='myFunction()''>"  # noqa
    if request.method == 'POST':
        if request.form['name']:
...
@app.route(
    '/category/<int:category_id>/catalog/<int:catalog_id>/delete',
    methods=['GET', 'POST'])
def deleteCatalogItem(category_id, catalog_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete catalog items to this category. Please create your own category in order to delete items.');}</script><body onload='myFunction()''>"  # noqa
    if request.method == 'POST':
